...
for i = 1:length(run_params_set)
    ...
    for dur_str = ["15s", "60s"]
        ...
        for s = participant_pool
            ...
            block_temp = load(mat_file).block2.rn.Accuracy('mean');
            block_hs = [block_hs; block_temp];
            block_temp = load(mat_file).block2.ln.Accuracy('mean');
...
arguments
    ...
    active_cell = change_excel_addr(cell_col, cell_row, 1, -1);
    writematrix("15s Trial", file_name, 'Sheet', "random", 'Range', active_cell);

    active_cell = change_excel_addr(cell_col, cell_row, 3, -1);
    writematrix("60s Trial", file_name, 'Sheet', "random", 'Range', active_cell);

    header_str = row_name + "/" + col_name;
    active_cell = change_excel_addr(cell_col, cell_row, -1, -1);
    writematrix(header_str, file_name, 'Sheet', "random", 'Range', active_cell);
