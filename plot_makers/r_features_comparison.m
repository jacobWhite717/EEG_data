...
for i = 1:length(feature_cat_options)
    ...
    [svm_k_m, svm_k_s, svm_b_m, svm_b_s, svm_b2_m, svm_b2_s] = get_rand_clasifier_feature_data(subject_range, @fitcsvm, feature_cat, classification_range);
    [discr_k_m, discr_k_s, discr_b_m, discr_b_s, discr_b2_m, discr_b2_s] = get_rand_clasifier_feature_data(subject_range, @fitcdiscr, feature_cat, classification_range);
    [knn_k_m, knn_k_s, knn_b_m, knn_b_s, knn_b2_m, knn_b2_s] = get_rand_clasifier_feature_data(subject_range, @fitcknn, feature_cat, classification_range);
...
title_str = sprintf("%s Classification (Randomized Labels) Feature Performance Comparison", classification_name);
title(title_str)
...
if save_flag
    ...
    plot_savename = char(sprintf("results/plots/all_feat_comparison/r %s.png", strrep(classification_name, '/', '_')));
    saveas(f , plot_savename);
end

%% funcs
function [kfold_means, kfold_stds, block_means, block_stds, block2_means, block2_stds] = get_rand_clasifier_feature_data(subject_range, classifier_func_handle, feature_cat, data_range)
    arguments
