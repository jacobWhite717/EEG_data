...
save_flag = 0;
%% get stats
feature_cat = "variance"; % all/bandpower/mean/rms/variance
subject_range = 5:13;

[svm_k_m, svm_k_s, svm_b_m, svm_b_s] = get_clasifier_feature_data(subject_range, @fitcsvm, feature_cat);
[discr_k_m, discr_k_s, discr_b_m, discr_b_s] = get_clasifier_feature_data(subject_range, @fitcdiscr, feature_cat);
[knn_k_m, knn_k_s, knn_b_m, knn_b_s] = get_clasifier_feature_data(subject_range, @fitcknn, feature_cat);
...
title_str = sprintf("Classification (True Labels) Using %s Features", feature_cat_title);
title(title_str)
...
if save_flag
    ...
    plot_savename = char(sprintf("results/plots/classifier_averaged/s %s.png", feature_cat));
    saveas(f , plot_savename);
end

%%
function [kfold_means, kfold_stds, block_means, block_stds] = get_clasifier_feature_data(subject_range, classifier_func_handle, feature_cat)
    arguments
