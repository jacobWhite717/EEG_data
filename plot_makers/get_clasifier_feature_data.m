function [kfold_means, kfold_stds, block_means, block_stds] = get_clasifier_feature_data(data_folder, subject_range, classifier_func_handle, feature_cat, data_range)
    arguments
        data_folder
    ...
    for i = 1:length(run_params_set)
        ...
        for s = participant_pool
            mat_name = ['results/participant/', data_folder, char(sprintf("/%i/%s/%s/", s, feature_catagory_str, classifier_str)), file_str, '.mat'];
